...
// M5 adds the OUT-CALL route: eded_vocab(fn, a, b) reaches the host bridge
// (which routes UI vocabulary requests back into the core), and eded_text(s)
// / eded_text_len() move string bytes into linear memory so the host can copy
// them across the separate-memory boundary (M4 scratch-copy generalized).
...
// text scratch: JSI strings live in the SH runtime, NOT the linear memory the
// host reads; eded_text(s) copies bytes here and hands back the address.
static char g_text[256] __attribute__((aligned(16)));
static int g_text_len = 0;

EM_JS(int, host_vocab_js, (int fn, int a, int b), {
  return globalThis["__ededHost"].vocab(fn, a, b);
});
...
static jsi::Value hfVocab(jsi::Runtime &, const jsi::Value &,
                          const jsi::Value *args, size_t count) {
  if (count < 3)
    return jsi::Value(-98);
  return jsi::Value(host_vocab_js((int)args[0].asNumber(),
                                  (int)args[1].asNumber(),
                                  (int)args[2].asNumber()));
}

static jsi::Value hfText(jsi::Runtime &rt, const jsi::Value &,
                         const jsi::Value *args, size_t count) {
  if (count < 1 || !args[0].isString())
    return jsi::Value(-98);
  auto s = args[0].asString(rt).utf8(rt);
  if (s.size() >= sizeof(g_text))
    s.resize(sizeof(g_text) - 1);
  memcpy(g_text, s.data(), s.size());
  g_text_len = (int)s.size();
  return jsi::Value((int)(intptr_t)g_text);
}

static jsi::Value hfTextLen(jsi::Runtime &, const jsi::Value &,
                            const jsi::Value *, size_t) {
  return jsi::Value(g_text_len);
}
...
int plugin_init(void) {
  ...
  attach("eded_vocab", 3, hfVocab);
  attach("eded_text", 1, hfText);
  attach("eded_text_len", 0, hfTextLen);
